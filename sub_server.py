...
def get_packet_times(pcap_file):
    ...
    if len(packets) > 0:
        # 첫 번째 패킷과 마지막 패킷의 시간 추출 (EDecimal 타입을 float으로 변환)
        start_timestamp = float(packets[0].time)
...
@app.route('/batch/receive_prediction', methods=['POST'])
def receive_prediction():
    # JSON 본문에서 데이터 추출
    data = request.get_json()
    
    # 각 데이터 가져오기
    selected_date = data.get('date')  # 클라이언트에서 전송한 날짜를 받음
    start_time = data.get('start_time')  # 선택된 시작 시간
    end_time = data.get('end_time')      # 선택된 종료 시간

    if not selected_date:
        return jsonify({'error': '날짜를 선택해주세요.'}), 400
    
    # predict 함수를 호출하여 결과를 받음
    traffic_count, app_count, prediction_messages = predict(selected_date, start_time, end_time)
    traffic_count = {int(k): v for k, v in traffic_count.items()}
    app_count = {int(k): v for k, v in app_count.items()}

    print("receive_prediction traffic : ", traffic_count)
    print("receive_prediction app : ", app_count)
    # 결과 반환
    return jsonify({
...
# predict 함수를 내부 호출 또는 API 호출 방식 모두에서 사용 가능하도록 수정
def predict(selected_date, start_time=None, end_time=None):
    ...
    if start_time and end_time:
        ...
        pcap_files = [
            ...
            if f.endswith('.pcap') and len(f) >= len(f"{pcap_folder}/{selected_date}") + 4 and
...
def get_protocol_counts(date, start_time, end_time):
    pcap_folder = os.getenv("PCAP_FOLDER")  # pcap 파일이 있는 폴더 경로
    pcap_files = glob.glob(f'{pcap_folder}/{date}*.pcap')  # 날짜에 해당하는 모든 pcap 파일
    print(f"pcap folder path : {pcap_folder}")
    print(f"date : {date}")
...
@app.route('/batch/protocol_rate', methods=['GET', 'OPTIONS'])
def protocol_counts():
    ...
    print("protocol_rate : ",protocol_counts)
    return jsonify(protocol_counts)

@app.route('/batch/send_pie', methods=['GET','OPTIONS'])
def send_pie():
    if request.method == 'OPTIONS':
    # Preflight 요청 처리
        return '', 200
    ...
    print("send_pie traffic : ", traffic_ratios)
    print("send_pie app : ", app_ratios)
